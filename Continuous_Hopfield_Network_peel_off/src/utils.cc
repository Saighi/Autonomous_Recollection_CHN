...
void show_vector_int_grid(std::vector<int> vec, int rows)
{
    int iter = 0;
    // std::cout << "activity :" << std::endl;
    // for (const auto &element : net.activity_list)
...
void show_matrix_double(std::vector<std::vector<double>> matrix)
{
...
void show_matrix_int(std::vector<std::vector<int>> matrix)
{
    for (const auto &row : matrix)
    {
        for (const auto &element : row)
...
void show_matrix_bool(std::vector<std::vector<bool>> matrix)
{
    for (const auto &row : matrix)
    {
        for (const auto &element : row)
...
std::vector<bool> take_peal(std::vector<int>& actual_overlay, std::vector<bool>& explored_state)
{
    std::vector<bool> mask = std::vector<bool>(actual_overlay.size(), false);
    for(int i = 0; i<actual_overlay.size(); i++ ){
        if (actual_overlay[i] > 0){
            if (explored_state[i] == 1)
            {
                mask[i]=1;
            }
        }
...
std::vector<int> peal_off(std::vector<int>& actual_overlay, std::vector<bool>& mask)
{
    for (int i = 0; i < actual_overlay.size(); i++)
    {
        if (mask[i] == 1)
    ...
    return actual_overlay;
}

std::vector<std::vector<int>> peal_off_syn(std::vector<std::vector<int>> &overlay_synapses, std::vector<bool> &explored_state)
{
    for (int i = 0; i < explored_state.size(); i++){
        for (int j = 0; j < explored_state.size(); j++)
        {
            if (explored_state[i] && explored_state[j]){
                if (overlay_synapses[i][j]>0){
                    overlay_synapses[i][j]-=1;
                }
            }
        }
    }
    return overlay_synapses;
}

std::vector<std::vector<bool>> toBinaryMatrix(std::vector<std::vector<int>> &overlay_synapses)
{
    std::vector<std::vector<bool>> binaryMatrix = std::vector<std::vector<bool>>(overlay_synapses.size(), std::vector<bool>(overlay_synapses.size(),0));

    for (size_t i = 0; i < overlay_synapses.size(); ++i)
    {
        for (size_t j = 0; j < overlay_synapses[i].size(); ++j)
        {
            binaryMatrix[i][j] = overlay_synapses[i][j] != 0 ? true : false;
        }
    }
