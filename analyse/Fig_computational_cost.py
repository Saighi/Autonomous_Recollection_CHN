...
myDir = "../../data/all_data_splited/trained_networks_fast/Fig_load_SR_average_new_inh_plas_big_simulations_many_correlations_new_convergence_1_iter"
...
fig, axes = plt.subplots(1, len(noise_levels), figsize=(3 * len(noise_levels), 3),sharey=True)
...
y_tick_indices = get_spaced_indices(len(all_num_patterns), 7)
...
for i, noise_level in enumerate(noise_levels):
    ...
    # axes[i].set_title(r'ρ = {noise_level:.2f}', 
    #                   fontsize=14, pad=10)
    
    # Set y-axis label only on first subplot to avoid redundancy
    if i == 0:  # First subplot
        axes[i].set_ylabel('Number of patterns',labelpad=15)
...
cbar_ax = fig.add_axes([1, 0.2, 0.015, 0.65])
cbar = fig.colorbar(im, cax=cbar_ax)
cbar.set_label('Log10(nb Iter)')
...
plt.tight_layout()
plt.savefig('plots/Fig_computational_cost.png', dpi=300, bbox_inches='tight')
plt.show()
#%%