...
myDir = "../../data/all_data_splited/sleep_simulations/Fig_load_SR_many_correlation_diag_inh_sleep"
...
correlation_to_plot = [0.1,0.25,0.5,0.75,1]        # or slice / mask as you like
...
for noise_level in correlation_to_plot:
    sub = df_last.loc[df_last["noise_level"] == noise_level].copy()
...
for correlation in correlation_to_plot:
...
cmap_iter.set_bad(color="lightgrey")                    # NaNs → grey
# 3️⃣  make the figure
r=1.1
...
cbar1_ax = fig.add_axes([0.92, 0.56, 0.01, 0.3])
...
cbar2_ax = fig.add_axes([0.92, 0.14, 0.01, 0.3])
...
plt.savefig("./plots/Fig_load_SR_average_correlations_diag_inh.png",dpi=300)
