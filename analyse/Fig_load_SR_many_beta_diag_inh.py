...
myDir = "../../data/all_data_splited/sleep_simulations/Fig_load_SR_many_beta_diag_ing_sleep"
...
for i, beta in enumerate(betas_to_plot):
    ...
    # ── first row: % simulations without errors ────────────────────────────
    pt_err = sub.pivot_table(
        values="is_not_error_before_all_fnd",
        index="num_patterns",
        columns="network_size",
    ...
    axes[0, i].set_title(rf"$\beta={beta}$", fontsize=15)
    axes[0, i].invert_yaxis()
    axes[0, i].grid(False)

    # ── second row: first iteration all patterns found ─────────────────────
    pt_itr = sub.pivot_table(
        values="first_iter_all_fnd",
        index="num_patterns",
        columns="network_size",
    )
    im2 = axes[1, i].imshow(pt_itr,
...
# 5️⃣  shared colour-bars
cb1_ax = fig.add_axes([0.92, 0.55, 0.02, 0.30])
fig.colorbar(im1, cax=cb1_ax)

cb2_ax = fig.add_axes([0.92, 0.15, 0.02, 0.30])
fig.colorbar(im2, cax=cb2_ax)

# 6️⃣  labels & layout
fig.text(0.52, 0.1, "Network size", ha="center", va="center")
fig.text(0.03, 0.51, "Number of patterns", ha="left", va="center", rotation=90)
...
# 5️⃣  shared colour-bars
cb1_ax = fig.add_axes([0.92, 0.55, 0.02, 0.30])
fig.colorbar(im1, cax=cb1_ax, label="% sims without error")

cb2_ax = fig.add_axes([0.92, 0.15, 0.02, 0.30])
fig.colorbar(im2, cax=cb2_ax, label="first iteration")
