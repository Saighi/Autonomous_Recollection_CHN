...
# myDir = "../../data/all_data_splited/sleep_simulations/Fig_load_SR_average_new_inh_plas_big_simulations_2025_optimized_no_free_phase"
myDir = "../../data/all_data_splited/sleep_simulations/Fig_load_SR_average_new_inh_plas_big_simulations_2025_optimized_no_free_phase"
...
#%%
not_all_beta = all_beta
#%%
r = 1.1
fig, axes = plt.subplots(2, 1, figsize=(9/r, 8/r), sharey=True, sharex=True)
...
for i, beta in enumerate(not_all_beta):
    ...
    pivot_table = data_one_beta.pivot_table(values='is_not_error_before_all_fnd', 
                                          index='num_patterns', 
                                          columns='network_size')
    im1 = axes[0].imshow(pivot_table*100,vmin=0, vmax=global_max_error)
    ...
    axes[0].grid(False)
    axes[0].invert_yaxis()
    axes[0].set_title(r'$\beta$ = '+str(beta),fontsize=15)
    
    # Second row: First iteration
    pivot_table = data_one_beta.pivot_table(values='first_iter_all_fnd', 
                                          index='num_patterns', 
                                          columns='network_size')
    im2 = axes[1].imshow(pivot_table, vmin=0, vmax=global_max_iter)
