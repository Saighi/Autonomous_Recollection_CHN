...
myDir = "../../data/all_data_splited/sleep_simulations/Fig_load_SR_average_new_auto_stop_inh_plas_many_betta_larger_networks_smaller_delta"
...
data = data.loc[data.groupby("sim_ID")["query_iter"].idxmax()]
#%%
#%%
for n in all_num_patterns:
    for s in all_net_sizes:
        ...
        if any_error:
            data.loc[(data['network_size'] == s) & (data['num_patterns'] == n),'first_iter_all_fnd']=None
            data.loc[(data['network_size'] == s) & (data['num_patterns'] == n),'first_iter_all_fnd']=None
#%%
...
#%%
fig, axes = plt.subplots(1,3, figsize=(10,30), sharey=True)

pivot_table = data.pivot_table(values='is_error_before_all_fnd', index='num_patterns', columns='network_size')
im = axes[0].imshow(pivot_table*100,cmap="Reds")
axes[0].set_xticks(x_tick_indices,all_net_sizes[x_tick_indices])
axes[0].set_yticks(y_tick_indices,all_num_patterns[y_tick_indices])
plt.colorbar(im, ax=axes[0], shrink=0.1)

pivot_table = data.pivot_table(values='first_iter_all_fnd', index='num_patterns', columns='network_size')
cmap = plt.cm.viridis.copy()
# cmap.set_bad('gray')
im = axes[1].imshow(pivot_table)
axes[1].set_xticks(x_tick_indices,all_net_sizes[x_tick_indices])
axes[1].set_yticks(y_tick_indices,all_num_patterns[y_tick_indices])
plt.colorbar(im, ax=axes[1], shrink=0.1)
data["first_iter_all_fnd_normalized"] = data['first_iter_all_fnd']/data["num_patterns"]

pivot_table = data.pivot_table(values='first_iter_all_fnd_normalized', index='num_patterns', columns='network_size')
# cmap = plt.cm.viridis.copy()
# cmap.set_bad('gray')
im = axes[2].imshow(pivot_table)
axes[2].set_xticks(x_tick_indices,all_net_sizes[x_tick_indices])
axes[2].set_yticks(y_tick_indices,all_num_patterns[y_tick_indices])
plt.colorbar(im, ax=axes[2], shrink=0.1)
