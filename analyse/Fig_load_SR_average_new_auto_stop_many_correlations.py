...
all_correlations = np.sort(data['noise_level'].unique())
#%%
...
for i,noise in enumerate(all_correlations):

    data_one_noise = data[data['beta']==noise]

    sim_IDs=np.sort(data['sim_ID'].unique())
    for sim_id in sim_IDs:
        is_error = False
        mask = data_one_noise['sim_ID']==sim_id
        sim_data = data_one_noise[mask]
        ...
        data_one_noise.loc[mask, 'is_error_before_all_fnd'] = is_error
        data_one_noise.loc[mask, 'first_iter_all_fnd'] = first_iter_all_fnd

    # Keep only the last iteration as representent of the simulation
    data_one_noise = data_one_noise.loc[data_one_noise.groupby("sim_ID")["query_iter"].idxmax()]
    for n in all_num_patterns:
        for s in all_net_sizes:
            is_error = data_one_noise.loc[(data_one_noise['network_size'] == s) & (data_one_noise['num_patterns'] == n),"is_error_before_all_fnd"]
            any_error = np.any(list(is_error))
            if any_error:
                data_one_noise.loc[(data_one_noise['network_size'] == s) & (data_one_noise['num_patterns'] == n),'first_iter_all_fnd']=None
                data_one_noise.loc[(data_one_noise['network_size'] == s) & (data_one_noise['num_patterns'] == n),'first_iter_all_fnd']=None
    data_one_noise["first_iter_all_fnd_normalized"] = data_one_noise['first_iter_all_fnd']/data_one_noise["num_patterns"]
    
    pivot_table = data_one_noise.pivot_table(values='is_error_before_all_fnd', index='num_patterns', columns='network_size')
    ...
    pivot_table = data_one_noise.pivot_table(values='first_iter_all_fnd_normalized', index='num_patterns', columns='network_size')
