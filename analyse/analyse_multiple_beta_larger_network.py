...
binary_patterns = []
for pat in patterns:
    binary_patterns.append([1 if i>0.5  else 0 for i in pat])
binary_patterns = np.array(binary_patterns)
# %%
winners_all_phases = []
for h in range(nb_betas):
    winners_all_phases.append([])
    for i in range(nb_phases):
        winners_all_phases[-1].append([1 if i>0.5  else 0 for i in data_sleep_phases[h][i][-1]])
winners_all_phases = np.array(winners_all_phases)
# %%
all_betas_cpt = []
for h in range(nb_betas):
    cpt = 0
    ...
    for i in range(nb_phases):
        ...
        for e in encountered:
            if np.all(winners_all_phases[h][i] == e):
                is_encountered = True
        if not is_encountered:
            for pat in binary_patterns:
                if np.all(winners_all_phases[h][i] == pat):
                    encountered.append(pat)
                    cpt+=1
    all_betas_cpt.append(cpt)
# %%
all_betas_cpt
# %%
