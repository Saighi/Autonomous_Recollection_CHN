...
myDir = "../../data/all_data_splited/sleep_simulations/Fig_load_SR_average_many_epsilon"
...
all_epsilon = np.sort(data['epsilon_learning'].unique())
...
#%%
data_epsilons_1 = dict()
for i,epsilon in enumerate(all_epsilon):
    data_one_epsilon = data[data['epsilon_learning']==epsilon]
    sim_IDs=np.sort(data['sim_ID'].unique())
    for sim_id in sim_IDs:
        is_error = False
        mask = data_one_epsilon['sim_ID']==sim_id
        sim_data = data_one_epsilon[mask]
        ...
        data_one_epsilon.loc[mask, 'is_error_before_all_fnd'] = is_error
        data_one_epsilon.loc[mask, 'first_iter_all_fnd'] = first_iter_all_fnd
        data_one_epsilon.loc[mask, 'noned'] = 1
        data_one_epsilon.loc[mask, 'more_than_one_noned'] = 1
    data_epsilons_1[epsilon]=data_one_epsilon
data_epsilons = dict()
#%%
for i,epsilon in enumerate(all_epsilon):
    data_one_epsilon=data_epsilons_1[epsilon]
    # Keep only the last iteration as representent of the simulation
    data_one_epsilon = data_one_epsilon.loc[data_one_epsilon.groupby("sim_ID")["query_iter"].idxmax()]
    for n in all_num_patterns:
        for s in all_net_sizes:
            is_error = data_one_epsilon.loc[(data_one_epsilon['network_size'] == s) & (data_one_epsilon['num_patterns'] == n),"is_error_before_all_fnd"]
            ...
            if any_error:
                data_one_epsilon.loc[(data_one_epsilon['network_size'] == s) & (data_one_epsilon['num_patterns'] == n),'noned']=None
            if more_than_one_error:
                data_one_epsilon.loc[(data_one_epsilon['network_size'] == s) & (data_one_epsilon['num_patterns'] == n),'more_than_one_noned']=None

    data_one_epsilon["first_iter_all_fnd_normalized"] = data_one_epsilon['first_iter_all_fnd']/data_one_epsilon["num_patterns"]

    data_epsilons[epsilon]=data_one_epsilon # store the specific data for later
#%%
r = 1.1
fig, axes = plt.subplots(2,4,figsize=(20/r,8/r), sharey=True,sharex=True)
for i, epsilon in enumerate(all_epsilon):
    data_one_epsilon = data_epsilons[epsilon]

    pivot_table = data_one_epsilon.pivot_table(values='is_error_before_all_fnd', index='num_patterns', columns='network_size')
    ...
    axes[0][i].set_title(r'$\rho$ = '+str(epsilon))

    plt.colorbar(im1, ax=axes[0][i], shrink=0.8)

    # FIRST ITER FIGS
    pivot_table = data_one_epsilon.pivot_table(values='first_iter_all_fnd_normalized', 
...
pivot_table = data_epsilons[0.5].pivot_table(
