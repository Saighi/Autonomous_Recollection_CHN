# %% [markdown]
# # McCallum 1995 Pseudorehearsal — Corrected Reproduction
#
# Corrections vs the naive protocol (`mccallum_1995_protocol.py`):
#
# 1. Base population BP=5 learned first
# 2. Pp = 2000 probes (not 300)
# 3. Relaxation r = 4N cycles
# 4. Inverse NOT accepted as stable (strict identity)
# 5. Heteroassociative noise only (no Gaussian input noise)
# 6. Symmetric weight updates
# 7. Synchronous delta learning (compute all outputs, then update)
#
# Also runs from the CLI:
# ```
# python mccallum_1995_corrected.py -N 200 --trials 5 --seed 42
# python mccallum_1995_corrected.py -N 50 100 200 --csv results.csv --no-plot
# ```
...
# ## Configuration
#
# Edit the cell below to change experiment parameters.
...
# ## Numba kernels
#
# All heavy work is JIT-compiled. These are pure functions that don't
...
def _train_delta_sync(weights, patterns, n_pat, new_idx,
    ...
    """Synchronous delta learning with symmetric weight updates.
    Heteroassociative noise applied only to the new pattern."""
    smooth = 1.0
...
# %% Single run
print("=" * 60)
print(f"Single run: N={cfg.network_size}, BP={cfg.base_pop}, "
...
ax.set_title(f"McCallum 1995 corrected — N={cfg.network_size} (single run)")
...
# Uncomment to export:
# ```python
# save_csv(results, "mccallum_1995_results.csv")
# ```
...
def _cli_main():
    ...
    p = argparse.ArgumentParser(
        description="McCallum 1995 pseudorehearsal (corrected)")
    ...
    for N in args.network_size:
        ...
        print("=" * 60)
        print(f"N={N}, BP={c.base_pop}, new={c.max_new}, "
        ...
        if not args.no_plot:
            save = None
            if args.plot_dir:
                save = Path(args.plot_dir) / f"mccallum_1995_N{N}.png"
            ...
            ax.set(xlabel="Patterns learned (M)", ylabel="Stable patterns")
            ax.set_title(f"McCallum 1995 — N={N}, {args.trials} trials")
            ax.grid(alpha=0.3)
            plt.tight_layout()
            if save:
