...
def compute_difference(df_homo, df_hetero, homo_sparsities, hetero_width, metric,
    ...
    # Average homogeneous conditions using nanmean (suppress warnings for all-NaN slices)
...
for col_idx, comparison in enumerate(COMPARISONS):
    ...
    # Update global limits (handle all-NaN case)
...
if SAVE_PLOTS:
    ...
    output_path = OUTPUT_DIR / "SR_homogeneous_vs_heterogeneous_comparison_claude.png"
...
# %%
print("homo 0.3 rows", len(filter_by_float(df_homo, "sparsity", 0.3)))
print("homo 0.7 rows", len(filter_by_float(df_homo, "sparsity", 0.7)))
print("hetero 0.4 rows", len(filter_by_float(df_hetero, "sparsity_width", 0.4)))
print("NaNs in success", np.isnan(diff_success.values).sum(), "/", diff_success.size)
