...
void run_training(
    ...
    if (native_mode) {
        ...
        if (use_heterogeneous) {
            // Heterogeneous sparsity: each pattern has different sparsity
            ...
            // Fixed sparsity mode (existing behavior)
...
int main(int argc, char** argv) {
    ...
    if (!config.native_pattern_generation) {
        ...
        // rho has a default, so it's optional
