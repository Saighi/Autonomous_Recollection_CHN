...
int main(int argc, char** argv) {
    ...
    // Load patterns
    std::vector<std::vector<bool>> patterns = loadPatterns(config.patterns_file);
    if (patterns.empty()) {
        std::cerr << "Error: No patterns loaded from " << config.patterns_file << std::endl;
        return 1;
    }

    std::cout << "Loaded " << patterns.size() << " patterns of size " << patterns[0].size() << std::endl;
    ...
    for (size_t sim_number = 0; sim_number < combinations.size(); ++sim_number) {
        ...
        threads.emplace_back([=, &mtx, &cv, &active_threads] {
            run_training(sim_number, combinations[sim_number], patterns, config.output_dir);
            {
