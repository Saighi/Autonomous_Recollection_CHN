...
void run_sleep(int sim_number, std::vector<std::vector<double>> net_weights, std::vector<std::vector<bool>> net_connectivity, const unordered_map<string, double> parameters, const string foldername_results, vector<vector<bool>> patterns)
{
    ...
    int nb_iter_max = static_cast<int>(parameters.at("nb_iter_mult_max") *
                                       parameters.at("num_patterns"));
...
int main(int argc, char **argv)
{
    ...
    string sim_name =
        "Fig_load_SR_average_new_inh_plas_big_simulations_many_correlations_diagonal_inh";
    string inputs_name =
        "Fig_load_SR_average_new_inh_plas_big_simulations_many_correlations";
