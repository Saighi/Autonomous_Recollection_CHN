...
void run_sleep(int sim_number, std::vector<std::vector<double>> net_weights, std::vector<std::vector<bool>> net_connectivity, const unordered_map<string, double> parameters, const string foldername_results, vector<vector<bool>> patterns)
{
    ...
    std::mt19937 rng(sim_number);
    std::bernoulli_distribution coin_flip(0.5);
    const double high_rate = 1.0 / (1.0 + std::exp(-6.0));  // net.transfer(6)
    const double low_rate = 1.0 / (1.0 + std::exp(6.0));    // net.transfer(-6)
    std::vector<double> random_state(network_size);

    int cpt = 0;
    int nb_iter_biased = 0;
    int nb_iter_free = 0;
    std::cout << "RETRIEVAL" << std::endl;
    while(cpt<2000 && nb_spurious_patterns==0 && !check)
    {
        // std::cout << "NEW ITER" << std::endl;
        for (auto &value : random_state)
        {
            value = coin_flip(rng) ? high_rate : low_rate;
        }
...
int main(int argc, char **argv)
{
    ...
    string sim_name = "Fig_load_SR_many_beta_diag_inh_sleep_random_initial_state";
    string inputs_name =
    ...
    vector<double> beta = {0.025,0.05,0.1,0.5,10};
