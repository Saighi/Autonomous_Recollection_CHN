...
void compute_and_save_potential_vector_field_two_pattern(
    ...
    double up_lim);
void run_net_sim_query_drive(Network &net, std::vector<double> &query_drives,
                             double strength_drive, int nb_iter, double delta);
...
void run_net_sim_noisy_save(Network &net, int nb_iter, double delta,
                            double mean, double stddev, std::ofstream &file);
void run_net_sim_save(Network &net, int nb_iter, double delta,
                      std::ofstream &file);
void run_net_sim_noisy_depressed(Network &, int, double, double, double);
void run_net_sim_noisy_depressed_save(Network &net, int nb_iter, double delta,
...
void run_net_sim_noisy_save_display(Network &net, int rows, int nb_iter,
                                    double delta, double mean, double stddev,
                                    std::ofstream &file);
struct Compare;
...
bool comparestates(const std::vector<bool> &state1,
                   const std::vector<bool> &state2);
std::vector<double> setToValueRandomElements(
    const std::vector<double> &baseValues, int numFlips, double value);
void lunchParalSim(
    std::string foldername_results,
    std::unordered_map<std::string, std::vector<double>> varying_params,
    void (*run_simulation)(int, std::unordered_map<std::string, double>,
                           const std::string));
