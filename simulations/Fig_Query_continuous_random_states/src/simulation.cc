...
void run_simulation(int sim_number, unordered_map<string, double> parameters, const string foldername_results)
{
    ...
    int network_size = IMAGE_HEIGHT*IMAGE_WIDTH;
    int nb_winners;
    double leak = parameters.at("leak");
    double delta = parameters.at("delta");
    int num_patterns = parameters.at("num_patterns");
    int start_pattern_index = parameters.at("start_pattern_index");
    int query_state_type = parameters.at("query_state_type"); // 0=neutral, 1=random
    int query_coverage = parameters.at("query_coverage"); // 0=partial, 1=full
    int num_random_queries = parameters.at("num_random_queries"); // Number of random queries when query_coverage=1
    ...
    for (size_t n = start_pattern_index; n < start_pattern_index + num_patterns; n++)
    {
        ...
        // Setup random number generator for random binary states
        std::random_device rd;
        std::mt19937 gen(rd());
        std::uniform_real_distribution<> dis(0.0, 1.0);

        // Calculate relative pattern index (0 for first pattern, 1 for second, etc.)
        int pattern_idx = n - start_pattern_index;

        for (size_t i = 0; i < initial_patterns[initial_patterns.size()-1].size(); i++)
        {
            bool is_informed_unit = false;

            // Determine if this unit should be informed (showing pattern) or uninformed
            if (query_coverage == 1) {
                // Full query: all units are uninformed
                is_informed_unit = false;
            } else {
                // Partial query: only some units are informed (depends on pattern and position)
                if (pattern_idx == 0) {
                    // Pattern 0: first 1/3 of units are informed
                    is_informed_unit = (i <= (initial_patterns[initial_patterns.size() - 1].size() / 3));
                } else if (pattern_idx == 1) {
                    // Pattern 1: last 1/3 of units are informed
                    is_informed_unit = (i >= (initial_patterns[initial_patterns.size() - 1].size() / 1.5));
                } else {
                    // Patterns 2+: all units informed (full pattern)
                    is_informed_unit = true;
                }
            }

            // Set the query pattern value based on whether unit is informed
            if (is_informed_unit) {
                // Informed unit: use the actual pattern value
                if(initial_patterns[initial_patterns.size()-1][i]==1){
                    query_patterns_rates[initial_patterns.size() - 1][i] = net.transfer(drive_target);
                } else {
                    query_patterns_rates[initial_patterns.size() - 1][i] = net.transfer(-drive_target);
                }
            } else {
                // Uninformed unit: use neutral or random state
                if (query_state_type == 0) {
                    // Neutral state
                    query_patterns_rates[initial_patterns.size() - 1][i] = 0.5;
                } else {
                    // Random binary state
                    double random_drive = (dis(gen) < 0.5) ? drive_target : -drive_target;
                    query_patterns_rates[initial_patterns.size() - 1][i] = net.transfer(random_drive);
                }
            }
        }
    ...
    // If using full random/neutral queries, regenerate query_patterns_rates
    if (query_coverage == 1) {
        // Clear existing query patterns
        query_patterns_rates.clear();
        ...
        // Generate num_random_queries random/neutral query patterns
        for (int q = 0; q < num_random_queries; q++) {
            vector<double> random_query(network_size, 0.0);
            ...
            for (int i = 0; i < network_size; i++) {
                if (query_state_type == 0) {
                    ...
                    // Random binary state
                    double random_drive = (dis(gen) < 0.5) ? drive_target : -drive_target;
                    random_query[i] = net.transfer(random_drive);
                }
            }

            query_patterns_rates.push_back(random_query);
        }
    }
    ...
    for (int i = 0; i < query_patterns_rates.size(); i++)
    {
        nb_winners=0;
        ...
        run_net_sim_save(net,50, delta, result_file_traj);
        ...
        // For partial queries, compare against corresponding pattern
        // For full random queries, check against all patterns to find best match
        if (query_coverage == 0) {
            // Partial query: 1:1 mapping with patterns
            ...
            // Full random query: check against all stored patterns
            bool matched = false;
            for (int p = 0; p < initial_patterns.size(); p++) {
                nb_winners = 0;
                for (size_t j = 0; j < initial_patterns[p].size(); j++)
                {
                    nb_winners+=initial_patterns[p][j];
                }
                winning_units = assignBoolToTopNValues(net.activity_list, nb_winners);
                if (comparestates(winning_units,initial_patterns[p])){
                    succes+=1;
                    matched = true;
...
int main(int argc, char **argv)
{
    ...
    string sim_name = "Fig_Query_continuous_random_states";
    ...
    unordered_map<string, vector<double>> varying_params = {
        ...
        {"delta", {0.02}},
        {"num_patterns", {2}},
        {"start_pattern_index", {3}},
        {"query_state_type", {0}},      // 0=neutral, 1=random
        {"query_coverage", {1}},        // 0=partial, 1=full
        {"num_random_queries", {1}}};  // Number of random queries when query_coverage=1
