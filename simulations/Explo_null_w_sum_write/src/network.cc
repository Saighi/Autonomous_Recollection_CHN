...
void Network::derivative_gradient_descent_soft_homeostasis(
    ...
    //-------------------------------------------------
    // 1) Precompute row sums to be used in homeostatic penalty
    //-------------------------------------------------
    std::vector<double> row_sums(size, 0.0);
    for (int i = 0; i < size; i++) {
        double sum_w = 0.0;
        for (int j = 0; j < size; j++) {
            if (connectivity_matrix[i][j] == 1)
    ...
    for (int i = 0; i < size; i++) {
        ...
        // Additional homeostatic penalty gradient
        //   = -homeo_coef * (row_sums[i])
        double grad_homeo = -homeo_coef * row_sums[i];

        for (int j = 0; j < size; j++) {
            if (connectivity_matrix[i][j] == 1) {
                double grad_pattern = learning_rate * 2 * diff * target_rates[j];
                update = grad_pattern + grad_homeo;
                weight_matrix[i][j] += update;
                weight_matrix[j][i] += update;
            }
        }
...
void Network::derivative_gradient_descent_with_momentum_null_sum(
    ...
    double momentum_coef, double alpha_homeo) {
    double input = 0;
    double ui;
    double update;
    double unit_target_drive;
