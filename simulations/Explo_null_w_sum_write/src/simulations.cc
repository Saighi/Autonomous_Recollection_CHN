...
void run_simulation(int sim_number, unordered_map<string, double> parameters,
    ...
    bool null_w_sum = parameters.at("null_w_sum");
    double alpha_homeo = parameters.at("alpha_homeo");
    ...
    while (max_error > epsilon_learning && cpt <= 10 / learning_rate) {
        for (int j = 0; j < initial_patterns.size(); j++) {
            ...
            if (null_w_sum) {
                ...
                //     initial_patterns[j], initial_patterns_rates[j],
                //     drive_target, learning_rate, leak, drives_error,
                //     velocity_matrix, momentum_coef, alpha_homeo);
                net.derivative_gradient_descent_soft_homeostasis(
                    initial_patterns[j], initial_patterns_rates[j],
                    drive_target, learning_rate, leak, drives_error,
                    alpha_homeo);
            } else {
                ...
                //     initial_patterns[j], initial_patterns_rates[j],
                //     drive_target, learning_rate, leak, drives_error,
                //     velocity_matrix, momentum_coef);
                net.derivative_gradient_descent(
                    initial_patterns[j], initial_patterns_rates[j],
                    drive_target, learning_rate, leak, drives_error);
            }
        }
...
int main(int argc, char **argv) {
    ...
    vector<double> num_patterns = {2.0};
    ...
    vector<double> noise_level = {1};
    double learning_rate = 0.0001;
    // double learning_rate= 0.00001; one night if 10 repetiotion, 50 to 300, 1,
    // 30 vector<double> noise_level = {0.5};
    // vector<double> repetition = generateEvenlySpacedIntegers(0, 20, 1);
    // vector<double> repetition = generateEvenlySpacedIntegers(0, 20, 2);
    vector<double> repetition = {1};
    // vector<double> repetition = generateEvenlySpacedIntegers(0,3,3);
    unordered_map<string, vector<double>> varying_params = {
        {"repetitions", repetition},
        {"ratio_flip_writing", {0.3}},
        {"drive_target", drive_targets},
        {"max_pattern",
         {*(std::max_element(num_patterns.begin(), num_patterns.end()))}},
