...
// Generate pattern states with uniform noise added to the distance from neutral state
std::vector<double> pattern_as_states_with_distance_noise(double drive_target, std::vector<bool> bin_pattern, double distance_noise_level, Network& net)
{
    ...
    for (int j = 0; j < state_input.size(); j++)
    {
        ...
        if (bin_pattern[j])
        {
            // Add uniform noise to the positive drive target
            noisy_drive = drive_target + dis(gen);
        }
        else
        {
            // Add uniform noise to the negative drive target
            noisy_drive = -drive_target + dis(gen);
        }
