...
void filterLastQueryIterations(const std::string &inputCsvPath,
                               const std::string &outputCsvPath) {
    std::ifstream inputFile(inputCsvPath);
    if (!inputFile.is_open()) {
        std::cerr << "Error: Unable to open input file: " << inputCsvPath
                  << std::endl;
        return;
    }
    ...
    while (std::getline(headerStream, header, ',')) {
        if (!header.empty() && header.back() == '\r') {
            header.pop_back();  // Remove potential carriage return
        }
        headers.push_back(header);
    }

    // Find indices for sim_ID and query_iter columns
    auto simIdIt = std::find(headers.begin(), headers.end(), "sim_ID");
    auto queryIterIt = std::find(headers.begin(), headers.end(), "query_iter");

    if (simIdIt == headers.end() || queryIterIt == headers.end()) {
        std::cerr << "Error: Required columns 'sim_ID' or 'query_iter' not "
                     "found in CSV"
                  << std::endl;
        return;
    }

    size_t simIdIndex = std::distance(headers.begin(), simIdIt);
    size_t queryIterIndex = std::distance(headers.begin(), queryIterIt);

    // Store data and find max query_iter for each sim_ID
    std::unordered_map<std::string, int>
        maxQueryIter;  // sim_ID -> max query_iter
    ...
    while (std::getline(inputFile, line)) {
        ...
        while (std::getline(lineStream, cell, ',')) {
            if (!cell.empty() && cell.back() == '\r') {
                cell.pop_back();  // Remove potential carriage return
            }
            row.push_back(cell);
        }

        if (row.size() >= std::max(simIdIndex, queryIterIndex) + 1) {
            std::string simId = row[simIdIndex];
            int queryIter;
            try {
                queryIter = std::stoi(row[queryIterIndex]);
            } catch (const std::exception &e) {
                std::cerr << "Error converting query_iter to integer: "
                          << row[queryIterIndex] << std::endl;
                continue;
            }

            auto it = maxQueryIter.find(simId);
            if (it == maxQueryIter.end() || queryIter > it->second) {
                maxQueryIter[simId] = queryIter;
            }
    ...
    if (!outputFile.is_open()) {
        std::cerr << "Error: Unable to open output file: " << outputCsvPath
                  << std::endl;
        return;
    }

    // Set maximum precision for floating-point numbers in output
    outputFile.precision(std::numeric_limits<double>::max_digits10);
    outputFile << std::fixed;  // Use fixed notation instead of scientific

    // Write header without modifying the original format
    outputFile << headerLine << "\n";

    // Write rows with max query_iter for each sim_ID
    for (const auto &row : allData) {
        std::string simId = row[simIdIndex];
        int queryIter = std::stoi(row[queryIterIndex]);

        if (queryIter == maxQueryIter[simId]) {
    ...
    outputFile.close();
    std::cout << "Successfully created filtered CSV with last queries at: "
              << outputCsvPath << std::endl;
}

void writeMatrixToFile(const std::vector<std::vector<double>> &matrix,
...
void writeBoolMatrixToFile(const std::vector<std::vector<bool>> &matrix,
                           const std::string &filePath) {
    std::ofstream outFile(filePath);
