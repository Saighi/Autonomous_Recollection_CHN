...
std::vector<double> generate_equal_spaced_values(double min, double max,
                                                       int num_points) {
    std::vector<double> values;
    values.reserve(num_points);

    // Use integers for index calculation to avoid floating-point errors
    for (int i = 0; i < num_points; i++) {
        // This gives perfect spacing
        double value = min + (max - min) * i / (num_points - 1.0);
...
void compute_and_save_potential_vector_field_two_pattern_negative_area(
    ...
    int size = net.size;  // e.g. 10
    std::vector<double> values = generate_equal_spaced_values(down_lim, up_lim, nb_step);
    // For i=0..nb_step-1, alpha = i/((nb_step-1)/up_lim)
    // For j=0..nb_step-1, beta  = j/((nb_step-1)/up_lim)
    // Then we store x=i, y=j in [0..nb_step-1] so Python can reshape.
    for (int i = 0; i < nb_step; i++) {
        double alpha = values[i];
        std::cout <<
            alpha << std::endl;
        for (int j = 0; j < nb_step; j++) {
            double beta = values[j];
            ...
            // 5) Write line: x, y, dot_x, dot_y, V0..V(size-1)
            file << std::fixed << std::setprecision(4) << alpha << " " << beta
                 << " " << dot_alpha << " " << dot_beta << " ";
            for (int k = 0; k < size; k++) {
                file << V[k];
                if (k < size - 1)
