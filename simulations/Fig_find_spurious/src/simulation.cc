...
void run_simulation(int sim_number, unordered_map<string, double> parameters, const string foldername_results)
{
    ...
    string result_file_traj_name =
        sim_data_foldername + "/results_" + to_string(which_pattern) + ".data";
    std::ofstream result_file_traj(result_file_traj_name, std::ios::trunc);

    //Complementary pattern to induce spurious state
    query_pattern=pattern_as_states(net.transfer(-drive_target), net.transfer(drive_target), query_patterns[which_pattern]);
...
int main(int argc, char **argv)
{
    ...
    string sim_name = "Fig_find_spurious";
