...
void run_net_sim_noisy_depressed_annealing(Network &net, int nb_iter, double delta, double mean, double stddev_start)
{
    int k=0;
    for (int i = 0; i < nb_iter; i++)
    {
        int t = 1-(k/nb_iter);
        net.noisy_depression_iterate(delta, mean, t*stddev_start);
        k+=1;    
    }
}
