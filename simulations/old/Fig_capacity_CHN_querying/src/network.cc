...
void Network::noisy_iterate(double delta, double mean, double stddev)
{
    ...
    // Generate noise for each element in advance
    std::vector<double> noise_list(size);
    std::generate(noise_list.begin(), noise_list.end(), [&]() { return distribution(generator); });
    ...
    for (int i = 0; i < size; i++)
    {
        ...
        derivative_activity_list[i] += noise_list[i]; // Adding noise to each derivative element
    }
