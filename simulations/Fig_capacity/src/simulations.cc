...
{    
    // Seed RNG from parameter if provided (for replicates), else use sim_number
    unsigned seed = parameters.count("seed") ? static_cast<unsigned>(parameters.at("seed"))
                                             : static_cast<unsigned>(sim_number);
    srand(seed);
    ...
    file.close();
    // Record parameters for this simulation (include resolved seed and nb_winners)
    parameters["seed"] = static_cast<double>(seed);
    ...
    // Querying: test recall of all stored patterns to assess capacity
    std::cout << "Querying initial memories" << std::endl;
    vector<double> query_pattern;
    int succes = 0;
    for (int i = 0; i < num_patterns; i++)
    {
        // Build a query state from the binary pattern and add perturbations on a subset of units
    ...
    result_file << "success_rate=" << (num_patterns > 0 ? (static_cast<double>(succes) / static_cast<double>(num_patterns)) : 0.0) << "\n";
    result_file.close();
...
int main(int argc, char **argv)
{
    ...
    const int max_threads = 20; // Limit concurrent threads
