...
// Helper function to compute the normalized dot product (cosine similarity)
...
void check_stable_states(Network net, std::vector<std::vector<bool>> patterns,
    ...
    net.set_state(std::vector<double>(net.size,init_drive));
    for (size_t i = 0; i < patterns.size(); i++)
    {
        std::vector<double> queryState =
            pattern_as_states(net.transfer(drive_target),
                              net.transfer(-drive_target), patterns[i]);
        net.set_state(queryState);
        run_net_sim_choice(net,config);
        double corr = computeCorrelation(net.rate_list,patterns[i]);
        *file<<corr<<" ";
    }
    *file<<std::endl;
    net.set_state(std::vector<double>(net.size, init_drive));
