...
int main(int argc, char **argv)
{
    ...
    string sim_name = "Fig_load_SR_average_new_inh_plas_many_betta_larger_networks_3_removed";
    ...
    for (const auto &path : all_paths)
    {
        ...
        double net_size = inherited_params.at("network_size");
        double nb_pat = inherited_params.at("num_patterns");
        bool in_ru = (net_size>size_ru)&&(nb_pat<nb_pat_ru); 
        bool in_b = (nb_pat>nb_pat_b); 
        bool in_lb = (net_size<size_lb)&&(nb_pat>nb_pat_lb); 
        if(!in_ru && !in_b && !in_lb){
