...
void Network::iterate(double delta) {
    for (int i = 0; i < size; i++) {
        for (int j = 0; j < size; j++) {
            // if (connectivity_matrix[i][j]==true){ // Will have to verify that
            // the weight stay 0 during the weight uptdate
    ...
    for (int i = 0; i < size; i++) {
        activity_list[i] +=
            delta * (derivative_activity_list[i] - (leak * activity_list[i]));
...
void Network::iterate_query_drive(double delta, double strength_drive,
                                  std::vector<double>& query_drives) {
    for (int i = 0; i < size; i++) {
        for (int j = 0; j < size; j++) {
            // if (connectivity_matrix[i][j]==true){ // Will have to verify that
            // the weight stay 0 during the weight uptdate
    ...
    for (int i = 0; i < size; i++) {
        activity_list[i] +=
            delta * (derivative_activity_list[i] - (leak * activity_list[i]) +
                     strength_drive * (query_drives[i] - activity_list[i]));
        rate_list[i] = transfer(activity_list[i]);
    };
...
void Network::noisy_iterate(double delta, double mean, double stddev) {
    ...
    std::normal_distribution<double> distribution(
        mean, stddev);  // Gaussian noise with user-specified mean and stddev

    for (int i = 0; i < size; i++) {
        double noise =
            distribution(generator);  // Generate Gaussian noise once per
                                      // iteration for each element
        for (int j = 0; j < size; j++) {
            // Apply the noise outside the inner loop, directly influencing each
            // derivative element only once
            derivative_activity_list[i] += (weight_matrix[i][j] * rate_list[j]);
        }
        derivative_activity_list[i] +=
            noise;  // Adding noise to each derivative element outside the inner
                    // loop
    }
...
void Network::noisy_depression_iterate(double delta, double mean,
                                       double stddev) {
    std::default_random_engine generator;
    std::normal_distribution<double> distribution(
        mean, stddev);  // Gaussian noise with user-specified mean and stddev
    ...
    for (int i = 0; i < size; i++) {
        ...
        for (int j = 0; j < size; j++) {
            // Apply the noise outside the inner loop, directly influencing each
            // derivative element only once the weight_matrix is added the
            // inhib_strenght to allow a lack of inhib from the inhib matrix to
            // be excitatory. there is no added inhibstrenght if connectivity is
            // not (maybe a better way).
            derivative_activity_list[i] +=
                ((weight_matrix[i][j] +
                  (inhib_strenght * connectivity_matrix[i][j]) -
                  inhib_matrix[i][j]) *
                 rate_list[j]);
        }
        derivative_activity_list[i] +=
...
void Network::blank_init() {
    ...
    weight_matrix =
        std::vector<std::vector<double>>(size, std::vector<double>(size));
    inhib_matrix =
        std::vector<std::vector<double>>(size, std::vector<double>(size));
...
void Network::rate_derivative_gradient_descent(std::vector<double> target_rate,
                                               double learning_rate,
                                               double leak) {
    double input = 0;
    double ui;
    ...
    for (int i = 0; i < size; i++) {
        ...
        for (int j = 0; j < size; j++) {
            if (connectivity_matrix[i][j] == 1) {
                update = (target_rate[i] - vi) * vi * (1 - vi) *
                         target_rate[i] * learning_rate;
                weight_matrix[i][j] += update;
                weight_matrix[j][i] += update;
            }
        }
...
// Increase inhibitory weights between winners, decrease weights from loosers to
// winners. Normalize all weights. The need to normalize comes from the fact
// that we may potentiate less than we depressed or the contrary base on the
// amount of winners/loosers. DOESNT WORK ! void
// Network::pot_inhib_normalize(double pot_rate, int nb_winners)
// {
//     double new_sum= 0.0;
//     std::vector<int> indexes_winners = findTopNIndexes(rate_list,
//     nb_winners);
...
//             if (connectivity_matrix[i][j] == 1)
//             {
//                 inhib_matrix[i][j] = (inhib_matrix[i][j] / new_sum) *
//                 sum_all_inhib;
//                 //inhib_matrix[j][i] = (inhib_matrix[i][j] / new_sum) *
//                 sum_all_inhib;
//             }
//         }
//     }
...
void Network::derivative_gradient_descent_anti_inverse_momentum(
    std::vector<bool>& target_bin_state, std::vector<double>& target_rates,
    double target_drive, double learning_rate, double leak,
    std::vector<double>& drive_errors,
    std::vector<std::vector<double>>& velocity_matrix, double momentum_coef,
    double anti_inverse_strength) {
    double input = 0;
    double ui;
    double update;
    double unit_target_drive;
    ...
    // First pass: Regular gradient descent with momentum
    for (int i = 0; i < size; i++) {
        for (int j = 0; j < size; j++) {
            if (connectivity_matrix[i][j] == 1) {
                input += weight_matrix[i][j] * target_rates[j];
        ...
        for (int j = 0; j < size; j++) {
            if (connectivity_matrix[i][j] == 1) {
                ...
                // Apply momentum for regular update
                velocity_matrix[i][j] =
                    momentum_coef * velocity_matrix[i][j] + update;
                velocity_matrix[j][i] =
                    velocity_matrix[i][j];  // Maintain symmetry
    ...
    // Second pass: Anti-Hebbian for inverse patterns
    // Scale anti_inverse_strength by current error magnitude to prevent
    // overshooting
    double error_scale =
        *std::max_element(drive_errors.begin(), drive_errors.end());
    double scaled_anti_strength =
        anti_inverse_strength *
        std::min(1.0, 1.0 / (std::abs(error_scale) + 1e-6));

    for (int i = 0; i < size; i++) {
        for (int j = 0; j < size; j++) {
            if (connectivity_matrix[i][j] == 1) {
                // Calculate inverse rates (only when needed)
                double inv_rate_i = target_rates[i] > 0.5 ? 0.0 : 1.0;
                double inv_rate_j = target_rates[j] > 0.5 ? 0.0 : 1.0;

                // Anti-Hebbian update with adaptive scaling
                double anti_update = -learning_rate * scaled_anti_strength *
                                     inv_rate_i * inv_rate_j;

                // Smaller momentum coefficient for anti-Hebbian term
                velocity_matrix[i][j] =
                    (momentum_coef * 0.5) * velocity_matrix[i][j] + anti_update;
