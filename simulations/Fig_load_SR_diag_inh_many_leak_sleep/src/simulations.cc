...
void run_sleep(int sim_number, std::vector<std::vector<double>> net_weights,
    ...
    std::ofstream result_file_retrieval(result_file_name_retrieval,
                                        std::ios::trunc);
    ...
    while (cpt < 200 && nb_spurious_patterns == 0 && !check) {
        ...
        if (save_trajectories) {
            result_file_name_sleep = sim_data_foldername + "/results_" +
                                     to_string(is_found_patterns.size()) +
                                     ".data";
        } else {
            result_file_name_sleep = "no_save.data";
        }
        std::ofstream result_file_sleep(result_file_name_sleep,
                                        std::ios::trunc);
        ...
        // Check if the output vector is in the target set and hasn't been
        // counted yet
        is_found_patterns.push_back(false);
        if (std::find(patterns.begin(), patterns.end(), winning_units) !=
            patterns.end()) {
    ...
    // result_file_retrieval << "iter_all_retrieved=" << iter_all_retrieved <<
    // std::endl;
    // // The number of unique vectors found
    // std::cout << "Number of unique vectors found: " << foundVectors.size() <<
    // " nb_patterns : " << num_patterns << " beta : " << beta << " nb_spurious
    // : " << nb_spurious_patterns << std::endl
    //           << " nb_winers : " << nb_winners << std::endl;
    std::cout << "sim_number " << sim_number << std::endl;
    std::cout << "nb_spurious :" << nb_spurious_patterns
              << " nb_found_patterns : " << foundVectors.size()
              << " nb_patterns : " << num_patterns << " beta : "
              << " nb_flip : " << " Network size: " << network_size
              << std::endl;
...
int main(int argc, char **argv) {
    ...
    string sim_name = "Fig_load_SR_diag_inh_many_leak_sleep";
    string inputs_name =
        "Fig_load_SR_many_leak_parameter";
