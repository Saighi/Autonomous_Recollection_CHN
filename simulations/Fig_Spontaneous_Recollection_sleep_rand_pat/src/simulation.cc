...
void run_sleep(int sim_number, std::vector<std::vector<double>> net_weights, std::vector<std::vector<bool>> net_connectivity, const unordered_map<string, double> parameters, const string foldername_results, vector<vector<bool>> patterns)
{
    ...
    while(cpt<nb_iter)
    {
        ...
        run_net_sim_noisy_depressed_convergence_check_save(net, delta/200, delta, 0.0, noise_stddev, result_file_sleep_depressed,100/delta); // Using utility function for noisy iterations and saving
        run_net_sim_noisy_convergence_check_save(net, delta/200, delta, 0.0, noise_stddev, result_file_sleep,100/delta); // Using utility function for noisy iterations
