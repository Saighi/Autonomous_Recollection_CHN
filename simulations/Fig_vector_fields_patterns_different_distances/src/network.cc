...
void Network::derivative_gradient_descent_arbitrary(std::vector<double>& target_rates,
                                          double learning_rate, double leak,
                                          std::vector<double>& drive_errors) {
    double input = 0;
    double ui;
...
// Synaptic push decomposition: excitatory-only (no leak, no inhibition)
std::vector<double> Network::give_derivative_u_excit_only(double delta) {
    std::vector<double> derivative_u(size, 0.0);
    ...
    for (int i = 0; i < size; i++) {
        ...
        double d = sum_weight - (leak * activity_list[i]);
        derivative_u[i] = d * delta;
    }
...
// Synaptic push decomposition: inhibitory-only (no leak, no excitation)
std::vector<double> Network::give_derivative_u_inhib_only(double delta) {
    std::vector<double> derivative_u(size, 0.0);

    for (int i = 0; i < size; i++) {
        double sum_inhib =
            avx_dot_product(inhib_matrix[i].data(), rate_list.data(), size);
        derivative_u[i] = (-sum_inhib) * delta;
    }
...
double Network::compute_energy() {
    ...
    // 1. Contribution from the weights (excitatory connections)
    for (int i = 0; i < size; i++) {
        for (int j = 0; j < size; j++) {
            if (connectivity_matrix[i][j]) {
                energy -=
                    0.5 * weight_matrix[i][j] * rate_list[i] * rate_list[j];
            }
        }
    }

    // 2. Contribution from inhibitory connections
    for (int i = 0; i < size; i++) {
        for (int j = 0; j < size; j++) {
            if (connectivity_matrix[i][j]) {
                energy +=
                    0.5 * inhib_matrix[i][j] * rate_list[i] * rate_list[j];
            }
        }
    }
    ...
    for (int i = 0; i < size; i++) {
        ...
        energy += 0.002 * leak * activity * activity;
    }
