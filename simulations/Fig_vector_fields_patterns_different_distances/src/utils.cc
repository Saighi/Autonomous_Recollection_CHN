...
// Variant: Save vector field using only the excitatory (classic/learned) synapses
void compute_and_save_potential_vector_field_two_pattern_excit_only(
    double delta, Network &net, const std::string &foldername,
    const std::string &filename,
    ...
    std::string out_filename = foldername +
                               "/vector_field_two_patterns_excit_only_" +
                               filename + ".txt";
    std::ofstream file(out_filename, std::ios::trunc);
    if (!file.is_open()) {
        std::cerr << "Cannot open file " << out_filename << std::endl;
    ...
    for (int i = 0; i < nb_step; i++) {
        ...
        for (int j = 0; j < nb_step; j++) {
            ...
            // Only excitatory push
            std::vector<double> dU = net.give_derivative_u_excit_only(delta);
    ...
    file.close();
    std::cout << "Saved excit-only 2D field to " << out_filename << std::endl;
}

// Variant: Save vector field using only the inhibitory synapses
void compute_and_save_potential_vector_field_two_pattern_inhib_only(
    double delta, Network &net, const std::string &foldername,
    const std::string &filename,
    const std::vector<double> &p1,
    const std::vector<double> &p2,
    ...
    double up_lim) {
    std::string out_filename = foldername +
                               "/vector_field_two_patterns_inhib_only_" +
                               filename + ".txt";
    std::ofstream file(out_filename, std::ios::trunc);
    if (!file.is_open()) {
        std::cerr << "Cannot open file " << out_filename << std::endl;
    ...
    for (int i = 0; i < nb_step; i++) {
        ...
        for (int j = 0; j < nb_step; j++) {
            ...
            // Only inhibitory push
            std::vector<double> dU = net.give_derivative_u_inhib_only(delta);
...
// Weights-only energy landscape: E = -0.5 * sum_ij w_ij * v_i * v_j (normalized)
void compute_and_save_energy_field_two_pattern_weights_only(
    Network &net, const std::string &foldername, const std::string &filename,
    const std::vector<double> &p1, const std::vector<double> &p2, int nb_step,
    double up_lim) {
    std::string out_filename = foldername +
                               "/energy_field_two_patterns_weights_only_" +
                               filename + ".txt";
    int size = net.size;
    ...
    for (int i = 0; i < nb_step; i++) {
        ...
        for (int j = 0; j < nb_step; j++) {
            ...
            for (int i2 = 0; i2 < size; i2++) {
                ...
                for (int j2 = 0; j2 < size; j2++) {
                    if (net.connectivity_matrix[i2][j2]) {
                        row += net.weight_matrix[i2][j2] * V[i2] * V[j2];
                    }
                }
                e += -0.5 * row;
            }
    ...
    file.close();
    std::cout << "Saved weights-only energy field (normalized) to "
              << out_filename << std::endl;
}
