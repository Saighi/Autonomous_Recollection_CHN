...
void run_sleep(int sim_number, std::vector<std::vector<double>> net_weights, std::vector<std::vector<bool>> net_connectivity, const unordered_map<string, double> parameters, const string foldername_results, vector<vector<bool>> patterns)
{
    ...
    mean_nb_found_patterns = nb_found_patterns/nb_repetition;

    result_file_retrieval <<"nb_found_patterns="<< mean_nb_found_patterns << std::endl;
    // The number of unique vectors found
    std::cout << "ratio random bits query: " << ratio_flips_querying <<" network size: " << network_size << " mean_found_patterns: " << mean_nb_found_patterns << " nb_patterns : " << num_patterns << std::endl;
...
{
    string sim_name = "Fig_capacity_CHN_5";
    string inputs_name = "Fig_capacity_CHN_5";
