...
void Network::blank_init()
{
    ...
    engram_matrix = std::vector<std::vector<bool>>(size,std::vector<bool>(size,false)); // No engrams at the begining of the simulation
}
...
void Network::reinforce_attractor_bistability(std::vector<double> target_state, double learning_rate , double treshold_bistability, double target_up)
{
    ...
    for (int i = 0; i < size; i++)
    {
        for (int j = 0; j < size; j++)
        {
            if (connectivity_matrix[i][j] == 1)
            {
                if (rate_list[j] > treshold_bistability){
                    actual_target = target_up;
                }else{
                    actual_target = 0.50;
                }
                double update = (actual_target - rate_list[j]) * target_state[i] * learning_rate;
                weight_matrix[i][j] += update;
                weight_matrix[j][i] += update;
            }
        }
...
void Network::reinforce_attractor_hebbian(std::vector<bool> target_state_binary, double learning_rate)
{
    double update;
    for (int i = 0; i < size; i++)
    {
        for (int j = 0; j < size; j++)
        {
            if (connectivity_matrix[i][j] == 1)
            {
                if (target_state_binary[i] && target_state_binary[j]){
                    update = learning_rate;
                }
                else if (!target_state_binary[i] && !target_state_binary[j])
                {
                    update = 0;
                }
                else
...
void Network::iterative_normalize_inhib(int nb_iter_normalize, double rate_normalize){
...
void Network::add_engram(std::vector<bool> engram){
    for (int i = 0; i<size;i++){
        for (int j = 0; j<size; j++)
        {
            if(engram[i] && engram[j]){
                engram_matrix[i][j] = true;
            }
        }
    }
}

void Network::potentiate_engrams(double pot_rate)
{
    for (int i = 0; i < size; i++)
    {
        for (int j = 0; j < size; j++)
        {
            if (engram_matrix[i][j])
            {
                weight_matrix[i][j]+=pot_rate;
            }
        }
    }
}