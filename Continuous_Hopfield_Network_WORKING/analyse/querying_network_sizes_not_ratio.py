...
# base_folder = "..\\..\\..\\data\\all_data_splited\\query_simulations\\quere_many_drives_relative_num_patterns_new_writing"
...
# %%
fig = plt.figure(layout='constrained', figsize=(7, 20))
    
ax = fig.subplots(len(relative_number_patterns),sharey=True)
...
for idx_2,nb_pat in enumerate(relative_number_patterns):
    for idx, size in enumerate(net_sizes):
        ...
        rat_flips = structured_data[:,0]
        fnd_pat = structured_data[:,1]
        ax[idx_2].plot(rat_flips, fnd_pat*size)
        ax[idx_2].set_xlabel("ratio flips")
        ax[idx_2].set_ylabel("total found patterns")
        ax[idx_2].set_title("number of pattern = "+ str(nb_pat))
