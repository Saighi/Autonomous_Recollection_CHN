...
int main(int argc, char **argv)
{
    ...
    for (int actual_sim = 0; actual_sim <sim_number; actual_sim++)
    {
        ...
        for (float beta : varying_params_double["betas"]) //looping over varying sleep parameters
        {
            ...
            {
                result_file_name_sleep = sim_sleep_data_foldername + "/results_"+to_string(r)+".data";
                std::ofstream result_file_sleep(result_file_name_sleep, std::ios::trunc);
                ...
                run_net_sim_noisy_depressed_save(net, 800, delta, 0.0, 0.01, result_file_sleep); // Using utility function for noisy iterations and saving
            ...
            result_file_name_retrieval = sim_sleep_data_foldername + "/results.data";
