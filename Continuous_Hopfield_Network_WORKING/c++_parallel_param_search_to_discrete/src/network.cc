...
Network::Network(std::vector<std::vector<bool>> connect_mat, int size_network, double lk, double stpnss)
{
    leak = lk;
...
// {
//     return (2.0 / (1.0 + std::exp(-steepness * activation))) - 1.0;
// }

// double Network::transfer_inverse(double activation)
// {
//     return -1.0 / steepness * std::log((1.0 - activation) / (1.0 + activation));
// }
...
void Network::derivative_gradient_descent(std::vector<bool>& target_bin_state,std::vector<double>& target_rates,double target_drive,double learning_rate, double leak, std::vector<double>& drive_errors)
{
    ...
    for (int i = 0; i < size; i++)
    {
        for(int j = 0; j < size; j++){
            if (connectivity_matrix[i][j] == 1) {
                inputs[i] += weight_matrix[i][j] * target_rates[j];
                }
        }
    }
    for (int i = 0; i < size; i++){
