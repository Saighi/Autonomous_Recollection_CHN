...
std::vector<double> Network::stochastic_run_annealing(const std::vector<double> &initial_state, int steps, double initial_temperature, double final_temperature)
{
    std::vector<double> state = initial_state;
    ...
    {
        // Linearly decrease temperature from initial_temperature to final_temperature
        double temperature = initial_temperature + (final_temperature - initial_temperature) * (double(step) / steps);
        ...
        for (int i = 0; i < size; ++i)
        {
            ...
            for (int j = 0; j < size; ++j)
            {
                h += weight_matrix[i][j] * state[j];
            }

            double probability = 1.0 / (1.0 + std::exp(-2.0 * h / temperature));
            state[i] = (dis(gen) < probability) ? 1.0 : -1.0;
        }
    }
    return state;
}

void Network::trainPerceptron(const std::vector<std::vector<double>> &patterns)
{
