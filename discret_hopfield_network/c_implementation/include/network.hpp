...
class Network{
    public:
        ...
        void trainPerceptron(const std::vector<std::vector<double>> &patterns, double step_size);
        void train(const std::vector<std::vector<double>> &patterns);
        std::vector<double> runReal(const std::vector<double> &initial_state, int nb_steps);
