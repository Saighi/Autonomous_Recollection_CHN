...
{
    //WRITING RESULTS
    std::string foldername = "../../data/spontaneous_recollection_neuronwise_adaptation";
    ...
    std::vector<double> betas = linspace(0.05, 0.5, 20);
    int number_iter = 20;
    // SIMULATION PARAMETERS
    ...
    for(int h = 0; h<betas.size();h++){
        ...
        for (int r = 0; r < number_iter; r++)
        {
            ...
            net.pot_depress_bin(betas[h], winning_units); // works with 0.005
            ...
            show_vector(net.depression);
        }
        net.reset_depression();
    }
    for(int h = 0; h<betas.size();h++){
