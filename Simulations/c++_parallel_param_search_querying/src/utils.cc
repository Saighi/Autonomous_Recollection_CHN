...
// int run_until_converg_net_sim_noisy(Network &net, double epsilon, double delta, double mean, double stddev)
// {
//     int nb_iter = 0;
//     int sampling_iter = 100;
//     int cpt = 0;
//     double rate_diff = 1.0;
//     std::vector<double> prev_rate_list = net.rate_list;
//     while (rate_diff > epsilon)
//     {
//         nb_iter+=1;
//         net.noisy_iterate(delta, mean, stddev);
//         if(cpt == sampling_iter){
//             rate_diff= std::accumulate(net.rate_list.begin(), net.rate_list.end(), 0.0) - std::accumulate(prev_rate_list.begin(), prev_rate_list.end(), 0.0);
//             prev_rate_list=net.rate_list;
//             cpt=0;
//         }
//         else{
//             cpt+=1;
...
std::vector<double> pattern_as_states(double up_rate, double down_rate, std::vector<bool> bin_patterns)
{
    std::vector<double> state_input(bin_patterns.size());
